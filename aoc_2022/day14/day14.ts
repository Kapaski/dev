...
const roll = (x: number, y: number): number[] => {
  ...
  if (sandMap[String(y+1)]?.includes( x)) {
    ...
    if (!sandMap[String(y+1)]?.includes( x-1) && !rockMap[String(y+1)]?.includes(x-1)){
    ...
    } else if (!sandMap[String(y+1)]?.includes( x+1) && !rockMap[String(y+1)]?.includes(x+1)) {
   ...
   if(rockMap[String(y+1)]?.includes(x)) {
      ...
      if(!rockMap[String(y+1)]?.includes(x-1) && !sandMap[String(y+1)]?.includes(x-1)) {
        //console.log('have rock - roll to down left ', x-1, y+1)
        return roll(x-1, y+1)
      // can roll down right
      } else if (!rockMap[String(y+1)]?.includes(x+1) && !sandMap[String(y+1)]?.includes(x+1)) { 
...
for(let yy = 0; yy <= yBoundary[1]; yy++) {
  let row = ''
  for(let xx = xBoundary[0]; xx<=xBoundary[1]; xx++) {
    if (xx == 500 && yy == 0) {
      row += 'o'
      continue
    }
